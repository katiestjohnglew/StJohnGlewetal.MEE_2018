...
blank <- raster(ncol=105, nrow=70, xmn=-13, xmx=8, ymn=48, ymx=62)
...
land<-subset(wrld_simpl,NAME %in% c("France", "Germany", "Netherlands", "Norway", "Belgium", "Denmark","United Kingdom", "Ireland", "Luxembourg"))
...
sea <- gDifference(outer, land) # don't worry about the warning here about different proj4 strings, I've no idea why it's saying that because it's not true!
plot(sea)
...
#setwd("D:/KSG/INLA")
#Template<-raster("Template_raster")
#Template_df<-as.data.frame(Template, xy=TRUE)
#Template_df<-na.omit(Template_df)
#x<-Template_df$x
#y<-Template_df$y
#Template_xy<-as.data.frame(cbind(x,y))

#pred_locs_df<-as.data.frame(pred_locs)
#colnames(pred_locs_df)<-c("x","y")
#Pred_locs_df2<-merge(Template_xy,pred_locs_df, by = c("x","y"),all=FALSE)
#pred_locs<-cbind(Pred_locs_df2$x,Pred_locs_df2$y)

#env_data_pred2<-merge(Template_xy,env_data_pred, by = c("x","y"),all=FALSE)
#env_data_pred<-env_data_pred2

# now link the predicted locations to the mesh (and limit to the boundary we set in the mesh section)
pred_grid <- inla.mesh.projector(mesh_bound, loc = pred_locs)
xy_in <- inout(pred_grid$loc, cbind(Jelly_bound[, 1], Jelly_bound[, 2])) # limits to boundary
...
s_stack_all <- inla.stack(s_stack_fit, stack_pred)

# 3. Spatial model
s_spatial_f <- y ~ -1 + Intercept + sst + mld + depth + chl + tdiff + bt + sal + f(Species, model = "iid") + f(w, model = spde_b)

#interactions - global model:

s_spatial_f <- y ~ -1 + Intercept + sst +  bt + mld + sal + depth + chl + tdiff + sst:bt + sst:sal + sst:mld + sst:depth + sst:chl + sst:tdiff + bt:sal + bt:mld + bt:depth +  bt:chl + bt:tdiff + sal:mld + sal:depth + sal:tdiff + mld:depth + mld:chl + mld:tdiff + depth:chl + depth:tdiff + chl:tdiff + chl:sal + f(Species, model = "iid") + f(w, model = spde_b)

s_spatial_m <- inla(s_spatial_f, 
                    family = "gaussian", 
                    data = inla.stack.data(s_stack_fit),
                    control.compute = list(dic = TRUE), 
                    control.predictor = list(A = inla.stack.A(s_stack_fit)))


#3.a. model outputs

summary(s_spatial_m)
Fit1 <- s_spatial_m$summary.fitted.values$mean[1:606]
E1   <- Jelly$d34S - Fit1

plot(Fit1,Jelly$d34S)
cor.test(Fit1,Jelly$d34S)
...
par(mfrow = c(1,1), mar = c(5,5,2,2), cex.lab = 1.5)
plot(x = Jelly$Species, 
     y = E1)
abline(h = 0)

s_spatial_m$summary.random

#Plot Random MArginals for S
plot(s_spatial_m$marginals.random$Species$index.1, col = "grey", type = "l", lwd = 4, ylim=c(0,30))
points(s_spatial_m$marginals.random$Species$index.2, col = "light blue", type = "l", lwd = 4)
points(s_spatial_m$marginals.random$Species$index.3, col = "goldenrod1", type = "l", lwd = 4)
points(s_spatial_m$marginals.random$Species$index.4, col = "darkseagreen2", type = "l", lwd = 4)
points(s_spatial_m$marginals.random$Species$index.5, col = "salmon1", type = "l", lwd = 4)
points(s_spatial_m$marginals.random$Species$index.6, col = "mediumpurple1", type = "l", lwd = 4)
points(s_spatial_m$marginals.random$Species$index.7, col = "pink1", type = "l", lwd = 4)


# 4. Predict
s_spatial_pred <- inla(s_spatial_f, 
                       family = "gaussian", 
                       data = inla.stack.data(s_stack_all),
                       control.compute = list(dic = TRUE), 
                       control.predictor = list(A = inla.stack.A(s_stack_all)),  
                       quantiles = NULL, 
...
id_pred <- inla.stack.index(s_stack_all, "Predicted")$data
s_mean <- s_spatial_pred$summary.fitted.values$mean[id_pred]
s_sd <- s_spatial_pred$summary.fitted.values$sd[id_pred]
s_res <- data.frame(pred_locs, s_mean, s_var = s_sd^2)

# 7. Plot the results - redo so plot as a raster 
s_mean_r<-subset(s_res, select = c("x", "y", "s_mean"))
coordinates(s_mean_r) <- ~x+y 
gridded(s_mean_r) <- TRUE
S_mean_raster<-raster(s_mean_r)

colfunc<-colorRampPalette(c("#a50026","#d73027","#f46d43","#fdae61","#fee090","#ffffbf","#e0f3f8","#abd9e9","#74add1","#4575b4","#313695"))
colfunc(100)
plot(S_mean_raster,col=colfunc(100), useRaster=FALSE)
points(Jelly_xy,pch=16,cex=1,col="black")
map('worldHires',xlim=c(-13,9),ylim=c(48,62), col="white", fill=TRUE, add =TRUE)

s_var_r<-subset(s_res, select = c("x", "y", "s_var"))
coordinates(s_var_r) <- ~x+y 
gridded(s_var_r) <- TRUE
S_var_raster<-raster(s_var_r)

plot(S_var_raster,col=colfunc(100), useRaster=FALSE)
points(Jelly_xy,pch=16,cex=1,col="black")
map('worldHires',xlim=c(-13,9),ylim=c(48,62), col="white", fill=TRUE, add =TRUE)
...
# 3. no interactions model
n_spatial_f <- y ~ -1 + Intercept + sst + mld + depth + chl + tdiff + bt + sal + f(Species, model = "iid") + f(w, model = spde_b)

#global model: 

n_spatial_f <- y ~ -1 + Intercept + sst +  bt + mld + sal + depth + chl + tdiff + sst:bt + sst:sal + sst:mld + sst:depth + sst:chl + sst:tdiff + bt:sal + bt:mld + bt:depth +  bt:chl + bt:tdiff + sal:mld + sal:depth + sal:tdiff + mld:depth + mld:chl + mld:tdiff + depth:chl + depth:tdiff + chl:tdiff + chl:sal + f(Species, model = "iid") + f(w, model = spde_b)

n_spatial_m <- inla(n_spatial_f, 
                    family = "gaussian", 
...
summary(n_spatial_m)
Fit1 <- n_spatial_m$summary.fitted.values$mean[1:606]
E1   <- Jelly$d15N - Fit1

plot(Fit1,Jelly$d15N)
cor.test(Fit1,Jelly$d15N)
...
n_spatial_m$summary.random

#Plot Random MArginals for n
plot(n_spatial_m$marginals.random$Species$index.1, col = "grey", type = "l", lwd = 4, ylim=c(0,1))
points(n_spatial_m$marginals.random$Species$index.2, col = "light blue", type = "l", lwd = 4)
points(n_spatial_m$marginals.random$Species$index.3, col = "goldenrod1", type = "l", lwd = 4)
points(n_spatial_m$marginals.random$Species$index.4, col = "darkseagreen2", type = "l", lwd = 4)
points(n_spatial_m$marginals.random$Species$index.5, col = "salmon1", type = "l", lwd = 4)
points(n_spatial_m$marginals.random$Species$index.6, col = "mediumpurple1", type = "l", lwd = 4)
points(n_spatial_m$marginals.random$Species$index.7, col = "pink1", type = "l", lwd = 4)

# 4. Predict
n_spatial_pred <- inla(n_spatial_f, 
                       family = "gaussian", 
...
colfunc<-colorRampPalette(c("#a50026","#d73027","#f46d43","#fdae61","#fee090","#ffffbf","#e0f3f8","#abd9e9","#74add1","#4575b4","#313695"))
colfunc(100)
plot(n_mean_raster,col=colfunc(100), useRaster=FALSE)
points(Jelly_xy,pch=16,cex=1,col="black")
map('worldHires',xlim=c(-13,9),ylim=c(48,62), col="white", fill=TRUE, add =TRUE)
...
plot(n_var_raster,col=colfunc(100),useRaster=FALSE)
points(Jelly_xy,pch=16,cex=1,col="black")
map('worldHires',xlim=c(-13,9),ylim=c(48,62), col="white", fill=TRUE, add =TRUE)
...
# 3. no interactions 
c_spatial_f <-y ~ -1 + Intercept + sst + mld + depth + chl + tdiff + bt + sal + f(Species, model = "iid") + f(w, model = spde_b)

# 3. Spatial model = global
c_spatial_f <- y ~ -1 + Intercept + sst + bt + tdiff + mld + sal + chl + depth +  sst:bt + sst:tdiff + sst:mld + sst:sal + sst:chl + 
  sst:depth + bt:tdiff + bt:mld + bt:sal + bt:chl + bt:depth + tdiff:mld + tdiff:sal + tdiff:chl + 
  mld:sal + mld:chl + mld:depth + sal:chl + sal:depth + chl:depth + depth:tdiff + f(Species, model = "iid") + f(w, model = spde_b)
...
Fit1 <- c_spatial_m$summary.fitted.values$mean[1:606]
E1   <- Jelly$d13Cc - Fit1
...
#Plot Random MArginals for n
plot(c_spatial_m$marginals.random$Species$index.1, col = "grey", type = "l", lwd = 4, ylim=c(0,50))
points(c_spatial_m$marginals.random$Species$index.2, col = "light blue", type = "l", lwd = 4)
points(c_spatial_m$marginals.random$Species$index.3, col = "goldenrod1", type = "l", lwd = 4)
points(c_spatial_m$marginals.random$Species$index.4, col = "darkseagreen2", type = "l", lwd = 4)
points(c_spatial_m$marginals.random$Species$index.5, col = "salmon1", type = "l", lwd = 4)
points(c_spatial_m$marginals.random$Species$index.6, col = "mediumpurple1", type = "l", lwd = 4)
points(c_spatial_m$marginals.random$Species$index.7, col = "pink1", type = "l", lwd = 4)
...
colfunc<-colorRampPalette(c("#a50026","#d73027","#f46d43","#fdae61","#fee090","#ffffbf","#e0f3f8","#abd9e9","#74add1","#4575b4","#313695"))
colfunc(100)
plot(c_mean_raster,col=colfunc(100), useRaster=FALSE)
points(Jelly_xy,pch=16,cex=1,col="black")
map('worldHires',xlim=c(-13,9),ylim=c(48,62), col="white", fill=TRUE, add =TRUE)
...
plot(c_var_raster,col=colfunc(100), useRaster=FALSE)
points(Jelly_xy,pch=16,cex=1,col="black")
map('worldHires',xlim=c(-13,9),ylim=c(48,62), col="white", fill=TRUE, add =TRUE)

#Cuts off area around Norway Channel: - save as raster to mask others by! 

values(c_mean_raster)[values(c_mean_raster)>-13.5]=NA
plot(c_mean_raster, col = colfunc(100),useRaster=FALSE)
map('worldHires',xlim=c(-13,9),ylim=c(48,62), col="white", fill=TRUE, add =TRUE)
...
writeRaster(c_mean_raster, "c_mean_raster_C2_res02_depthlim_global",overwrite=TRUE)
writeRaster(c_var_raster, "c_var_raster_C2_res02_depthlim_global",overwrite=TRUE)
writeRaster(n_mean_raster, "n_mean_raster_N1_res02_depthlim_global",overwrite=TRUE)
writeRaster(n_var_raster, "n_var_raster_N1_res02_depthlim_global",overwrite=TRUE)
writeRaster(S_mean_raster, "s_mean_raster_S1_res02_depthlim_global_interactions",overwrite=TRUE)
writeRaster(S_var_raster, "s_var_raster_S1_res02_depthlim_global_interactions",overwrite=TRUE)
...
writeRaster(c_mean_raster, "UK_c_mean_raster_C2_res02_depthlim_global",overwrite=TRUE)
writeRaster(c_var_raster, "UK_c_var_raster_C2_res02_depthlim_global",overwrite=TRUE)
writeRaster(n_mean_raster, "UK_n_mean_raster_N1_res02_depthlim_global",overwrite=TRUE)
writeRaster(n_var_raster, "UK_n_var_raster_N1_res02_depthlim_global",overwrite=TRUE)
writeRaster(S_mean_raster, "UK_s_mean_raster_S1_res02_depthlim_global_interactions",overwrite=TRUE)
writeRaster(S_var_raster, "UK_s_var_raster_S1_res02_depthlim_global_interactions",overwrite=TRUE)
