...
dataO<-dataO[dataO$year=="2010",]
dataO<-dataO[dataO$Sea.Area=="North.sea",]
#2010 = lots od samples, 2001 - not many samples! 

#setwd("/Users/ksjg1g08/Desktop/OneDrive - University of Southampton/PhD/Data_Analysis_R/MixedModelIsoscapes/UK_Shelf/Priors")
#Prior<-raster("NSPrior95")
#Priordf<-as.data.frame(rasterToPoints(Prior))
#Priorsub<-Priordf[Priordf$layer > 0.9,]
#Priorxy<-cbind(Priorsub$x,Priorsub$y)
...
spotX<-vector(length=ldat*length(odds))
spotY<-vector(length=ldat*length(odds))

names<-c("ID", "MC count","dist", "Al","PropArea", "cor","oddsS", "spotX", "spotY")
...
map('worldHires',xlim=c(-13,10),ylim=c(58,62), col="white", fill=TRUE, add =TRUE)
...
#####UPDATE ALL THESE 
#paper values - need to update
#Within species variance differences between species: 
#Barrel = 0.42 C, 0.97 N
#Blue = 0.62 C, 0.44 N
#Compass = 0.61 C, 0.53 N
#Crystal = 1.08 C, 1.65 N
#Lion = 0.60 C, 0.62 N
#Mauve = 0.48 C, 0.95 N
#Moon = 0.45 C, 0.62 N 
#gone for biggest values:
...
for(MC in 1:MCrep){
  ...
  for(i in 1:ldat){
    ...
    for(critical in 1:length(odds)){
      ...
      maps2<-matrix(NA, nrow=29400, ncol=length(s1$Station))
      ...
      #reverse rows and columns to what specify at beginning
      ...
      val=which(areaB==1)
      grd2 <- expand.grid(x=seq(from=-13, to=7.9, by=q), y=seq(from=48, to=61.9, by=q) )
...
result<-rbind(result, cbind(ID,MC.count, dist, Al, PropArea, cor, oddsS, spotX, spotY))
...
write.table(ResultsZ.df, "/Users/ksjg1g08/Desktop/OneDrive - University of Southampton/PhD/Data_Analysis_R/MixedModelIsoscapes/UK_Shelf/ResultsZ_INLA_UKShelf_2010_NSscallops_trial.txt", sep="\t")
