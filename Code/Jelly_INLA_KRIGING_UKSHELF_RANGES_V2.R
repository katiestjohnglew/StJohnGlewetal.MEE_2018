...
# 3. Spatial model
s_spatial_f <- y ~ -1 + Intercept + sst + mld + depth + chl + tdiff + f(Species, model = "iid") + f(w, model = spde_b)
...
plot(s_spatial_m$marginals.random$Species$index.1, col = "grey", type = "l", lwd = 4, ylim=c(0,4))
...
# 3. Spatial model
n_spatial_f <- y ~ -1 + Intercept + sst +  bt + sal + depth + chl + tdiff + sst:bt + sst:sal + sst:mld + sst:depth + sst:chl + sst:tdiff + bt:sal + bt:mld + bt:depth +  bt:chl + bt:tdiff + sal:mld + sal:depth + sal:tdiff + mld:depth + mld:tdiff + depth:chl + depth:tdiff + chl:tdiff + f(Species, model = "iid") + f(w, model = spde_b)
...
# 3. Spatial model
c_spatial_f <- y ~ -1 + Intercept + sst + bt + sal + depth + chl + tdiff + f(w, model = spde_b) + f(Species, model = "iid")
...
#Plot Random MArginals for n
plot(c_spatial_m$marginals.random$Species$index.1, col = "grey", type = "l", lwd = 4, ylim=c(0,2))
...
writeRaster(c_mean_raster, "c_mean_raster_C2_res02_depthlim",overwrite=TRUE)
writeRaster(c_var_raster, "c_var_raster_C2_res02_depthlim",overwrite=TRUE)
writeRaster(n_mean_raster, "n_mean_raster_N1_res02_depthlim",overwrite=TRUE)
writeRaster(n_var_raster, "n_var_raster_N1_res02_depthlim",overwrite=TRUE)
writeRaster(S_mean_raster, "s_mean_raster_S1_res02_depthlim",overwrite=TRUE)
writeRaster(S_var_raster, "s_var_raster_S1_res02_depthlim",overwrite=TRUE)
